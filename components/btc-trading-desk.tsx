...
export function BtcTradingDesk({ positionFeedUrl, performanceFeedUrl, rollingFeedUrl, dailyFeedUrl, compact = false }: Props) {
  ...
  return <div className={`trading-desk ${compact ? "td-compact" : ""}`}>
    ...
    <details className="td-methodology"><summary>Data, privacy & methodology <span aria-hidden="true">+</span></summary><div><p>These are sanitized account observations, not ASRA strategy results or an execution interface. All non-zero symbols and hedge sides remain visible in the public feed. Exact sizes, prices, per-position PnL, balances and credentials are not published. Open-order presence is not proof of a protective stop.</p><p>Account tracking begins at 2026-08-01T00:00:00Z. The source supplies flow-adjusted performance; the interface does not recompute it. After 180 seconds, observations are marked stale. A failed update never means that the account is flat.</p><p>Daily PnL and daily return are published read-only account telemetry derived from the authenticated flow-adjusted ledger. For historical ledger gaps from 2026-08-01 through 2026-09-22, the calendar may show a clearly labelled CSV realized-PnL fallback derived from the Binance transaction export; it includes realized PnL, funding and commissions, excludes transfers, and does not claim mark-to-market daily return. Personal notes stay browser-local and are never included in the public feed.</p>{metrics && <p className="td-mono td-hash">Performance source: {metrics.telemetry_sha256}</p>}</div></details>
  </div>;
}
...
function PublicDailyCalendar({ feedUrl }: { feedUrl: string }) {
  ...
  const dayLabel=(day?:CalendarPerformanceDay)=>day?.source==="CSV_REALIZED"?"CSV realized PnL":day?.status==="MISSING"?"Missing source":day?.status==="IN_PROGRESS"?"Live / incomplete":day?.status==="CLOSED"?"Closed UTC day":"Not published";
  return <section id="daily-journal" className="td-journal" aria-labelledby="td-journal-heading">
    ...
    <aside className="td-panel td-day-editor"><div className="td-editor-heading"><p className="td-kicker">DAY DETAIL / PUBLIC RESULT</p><h3>{selected?new Intl.DateTimeFormat("en-US",{weekday:"short",day:"numeric",month:"short",timeZone:"UTC"}).format(new Date(`${selected}T00:00:00Z`)):"Select a date"}</h3><p className="td-muted td-mono">{selected||"UTC"} / {dayLabel(selectedDay)}</p></div>
      ...
      {selectedDay?.source==="CSV_REALIZED"&&<p className="td-csv-explain">Historical Binance transaction-export fallback. This is realized PnL plus funding and commissions for the UTC day, with transfers excluded. It does not include unrealized mark-to-market change at the UTC boundary, so no daily return is shown.</p>}
