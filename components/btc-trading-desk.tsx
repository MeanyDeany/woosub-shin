...
import { parseBtcDailyPerformanceTelemetry } from "@/lib/btc-daily-performance";
import { applyCsvRealizedBackfill, type CalendarPerformanceDay } from "@/lib/btc-daily-csv-backfill";
  ...
  return <div className={`trading-desk ${compact ? "td-compact" : ""}`}>
    ...
    <div className="td-market-grid">
      <section className="td-panel td-positions" aria-labelledby="td-positions-heading">
        <div className="td-panel-heading"><div><h3 id="td-positions-heading">Open positions <span className="td-count">{positions.data ? items.length : "—"}</span></h3></div><Status label={feedState(positions, now)} /></div>
    ...
    <details className="td-methodology"><summary>Data, privacy & methodology <span aria-hidden="true">+</span></summary><div><p>These are sanitized account observations, not ASRA strategy results or an execution interface. All non-zero symbols and hedge sides remain visible in the public feed. Exact sizes, prices, per-position PnL, balances and credentials are not published. Open-order presence is not proof of a protective stop.</p><p>Account tracking begins at 2026-08-01T00:00:00Z. The source supplies flow-adjusted performance; the interface does not recompute it. After 180 seconds, observations are marked stale. A failed update never means that the account is flat.</p><p>Daily PnL and daily return are published read-only account telemetry derived from the authenticated flow-adjusted ledger. The public calendar extends back to the first supplied Binance futures transaction on 2024-11-15 using historical transaction exports. Historical cells show stablecoin realized cash PnL only: supported BTCUSDT/BTCUSDC REALIZED_PNL, FUNDING_FEE and USDT/USDC COMMISSION. Capital flows are excluded; BNB-denominated commission and ambiguous income types are not converted into USD and make that date visibly partial. From 2026-08-01 onward authenticated ledger rows take precedence and provide mark-to-market daily PnL and daily return when available. Personal notes stay browser-local and are never included in the public feed.</p>{metrics && <p className="td-mono td-hash">Performance source: {metrics.telemetry_sha256}</p>}</div></details>
  </div>;
}
...
function PublicDailyCalendar({ feedUrl }: { feedUrl: string }) {
  ...
  const displayDays=applyCsvRealizedBackfill(daily.data?.days??[]);
  const publicDays=new Map(displayDays.map(day=>[day.date_utc,day]));
  const monthDays=displayDays.filter(day=>day.date_utc.startsWith(month+"-")&&day.net_pnl!==null);
  const ledgerMeasured=monthDays.filter(day=>day.source==="LEDGER");
  const csvMeasured=monthDays.filter(day=>day.source==="CSV_REALIZED");
  const ledgerNet=ledgerMeasured.length?ledgerMeasured.reduce((sum,day)=>sum+(day.net_pnl??0),0):null;
  const csvNet=csvMeasured.length?csvMeasured.reduce((sum,day)=>sum+(day.net_pnl??0),0):null;
  const csvPartial=csvMeasured.filter(day=>day.source==="CSV_REALIZED"&&day.partial).length;
  ...
  const dayLabel=(day?:CalendarPerformanceDay)=>day?.source==="CSV_REALIZED"?(day.partial?"CSV partial realized PnL":"CSV realized PnL"):day?.status==="MISSING"?"Missing source":day?.status==="IN_PROGRESS"?"Live / incomplete":day?.status==="CLOSED"?"Ledger verified UTC day":"Not published";
  return <section id="daily-journal" className="td-journal" aria-labelledby="td-journal-heading">
    <div className="td-section-kicker"><span>Trading history</span><Status label={feedState(daily, calendarNow)} /></div>
    <div className="td-journal-intro"><div><h2 id="td-journal-heading">Trading history</h2><p>Daily UTC record from the first supplied Binance futures transaction, 15 Nov 2024.</p></div><div className="td-actions"><button className="td-button" onClick={()=>void daily.refresh()} disabled={daily.refreshing}>{daily.refreshing?"Updating":"Refresh daily"}</button><button className="td-button" onClick={exportBackup}>Export private notes <span aria-hidden="true">↓</span></button><button className="td-button" disabled={!ready} onClick={()=>fileInput.current?.click()}>Import notes</button><input className="td-sr-only" type="file" accept="application/json,.json" ref={fileInput} tabIndex={-1} aria-label="Import private note backup" onChange={e=>void importBackup(e.target.files?.[0])}/></div></div>
    <div className="td-journal-layout"><div className="td-panel td-calendar-panel"><div className="td-calendar-toolbar"><div><h3>{month?new Intl.DateTimeFormat("en-US",{month:"long",year:"numeric",timeZone:"UTC"}).format(new Date(`${month}-01T00:00:00Z`)):"Daily calendar"}</h3><span className="td-muted td-mono">PUBLIC ACCOUNT PERFORMANCE / UTC</span></div><div className="td-actions"><button className="td-button" onClick={()=>choose(TRACKING_START)} disabled={!ready}>Start</button><button className="td-button td-month-arrow" aria-label="Previous month" disabled={!month||month<=TRACKING_START.slice(0,7)} onClick={()=>setMonth(shiftMonth(month,-1))}>‹</button><button className="td-button" onClick={()=>{if(today===selected)setMonth(today.slice(0,7));else choose(today);}} disabled={!ready}>Today</button><button className="td-button td-month-arrow" aria-label="Next month" disabled={!month||month>=today.slice(0,7)} onClick={()=>setMonth(shiftMonth(month,1))}>›</button></div></div>
      <div className="td-month-summary"><SummaryItem label="Ledger day PnL"><span className={tone(ledgerNet)}>{money(ledgerNet)}</span></SummaryItem><SummaryItem label="CSV stablecoin realized"><span className={tone(csvNet)}>{money(csvNet)}</span></SummaryItem><SummaryItem label="Coverage">{ledgerMeasured.length}<span className="td-muted"> ledger / </span>{csvMeasured.length}<span className="td-muted"> CSV</span>{csvPartial>0&&<><span className="td-muted"> / </span>{csvPartial}<span className="td-muted"> partial</span></>}</SummaryItem></div>
      <div className="td-calendar-weekdays" aria-hidden="true">{["MON","TUE","WED","THU","FRI","SAT","SUN"].map(day=><span key={day}>{day}</span>)}</div>
      <div className="td-calendar-grid" aria-label="Public Binance USD-M daily performance calendar">{month?calendarDays(month).map(date=>{const day=publicDays.get(date);const privateNote=notes.get(date)?.note;const disabled=!ready||date<TRACKING_START||date>today;return <button type="button" key={date} data-journal-date={date} className={`td-day ${date.slice(0,7)!==month?"td-day-outside":""} ${date===selected?"td-day-selected":""} ${day?.source==="CSV_REALIZED"?"td-day-csv":""} ${day?.net_pnl!=null?day.net_pnl>0?"td-day-gain":day.net_pnl<0?"td-day-loss":"td-day-flat":""}`} disabled={disabled} aria-pressed={date===selected} aria-label={`${date}${date===today?", today":""}, ${day?.net_pnl!=null?`${money(day.net_pnl)} public net PnL`:dayLabel(day)}`} onClick={()=>choose(date)} onKeyDown={e=>{const steps:Record<string,number>={ArrowLeft:-1,ArrowRight:1,ArrowUp:-7,ArrowDown:7};if(!(e.key in steps))return;e.preventDefault();const next=new Date(Date.parse(`${date}T00:00:00Z`)+steps[e.key]*86_400_000).toISOString().slice(0,10);if(next<TRACKING_START||next>today)return;choose(next);window.requestAnimationFrame(()=>document.querySelector<HTMLButtonElement>(`[data-journal-date="${next}"]`)?.focus());}}><span className="td-day-number">{Number(date.slice(-2))}{date===today&&<i aria-label="Today"/>}</span>{day?.net_pnl!=null?<><strong className={tone(day.net_pnl)}>{money(day.net_pnl,true)}</strong><span className="td-no-result">{day.source==="CSV_REALIZED"?(day.partial?"CSV partial":"CSV realized"):<>{percent(day.return_pct)}{day.status==="IN_PROGRESS"?" · LIVE":""}</>}</span></>:<span className="td-no-result">{day?.status==="MISSING"?"?":"—"}</span>}{privateNote&&<span className="td-day-note" aria-label="Private note on this browser"><span aria-hidden="true">≡</span><span>Private note</span></span>}</button>; }):<p className="td-calendar-loading">Loading public daily performance…</p>}</div>
      <div className="td-calendar-legend"><span><i className="td-legend-gain"/>Positive</span><span><i className="td-legend-loss"/>Negative</span><span><i className="td-legend-csv"/>Historical CSV realized</span><span><i/>Missing / unpublished</span><span className="td-muted">{daily.error?"Latest daily feed unavailable; no values fabricated.":"Public read-only telemetry"}</span></div>
    </div>
    <aside className="td-panel td-day-editor"><div className="td-editor-heading"><h3>{selected?new Intl.DateTimeFormat("en-US",{weekday:"short",day:"numeric",month:"short",timeZone:"UTC"}).format(new Date(`${selected}T00:00:00Z`)):"Select a date"}</h3><p className="td-muted td-mono">{selected||"UTC"} / {dayLabel(selectedDay)}</p></div>
      <div className="td-public-day-values"><div><span>{selectedDay?.source==="CSV_REALIZED"?"CSV realized PnL":"Daily net PnL"}</span><strong className={tone(selectedDay?.net_pnl)}>{money(selectedDay?.net_pnl)}</strong></div><div><span>Daily return</span><strong className={tone(selectedDay?.return_pct)}>{selectedDay?.source==="CSV_REALIZED"?"Not available":percent(selectedDay?.return_pct)}</strong></div></div>
      {selectedDay?.source==="CSV_REALIZED"&&<p className="td-csv-explain">Historical Binance transaction-export value. Stablecoin realized cash PnL sums BTCUSDT/BTCUSDC REALIZED_PNL, FUNDING_FEE and USDT/USDC COMMISSION for the UTC day. Transfers and coin-swap flows are excluded. It does not include UTC-boundary unrealized mark-to-market change, so no daily return is shown.{selectedDay.partial?` Partial day: ${Math.abs(selectedDay.excluded_bnb_commission_bnb).toFixed(8)} BNB commission and ${selectedDay.ambiguous_event_count} ambiguous event(s) are excluded from the USD figure.`:""}</p>}
      ...
      <p className={`td-save-notice ${problem?"td-negative":""}`} role="status" aria-live="polite">{notice}</p><div className="td-private-note"><p>Public figures are identical for every visitor. Personal notes stay in this browser and are never uploaded.</p></div>
    </aside></div>
  </section>;
}
