...
try {
  for (const theme of ["dark", "light"]) for (const width of [1440, 390]) {
    ...
    // External data is unavailable by design in layout QA, not replaced by invented PnL.
    ...
    for (const pathname of siteRoutes) {
      ...
      const snapshot = await page.evaluate(() => {
        ...
        const heading = h1s[0];
        return { h1Count: h1s.length, heading: heading?.textContent, font: heading ? getComputedStyle(heading).fontFamily : null, ledgerHeading: Boolean(heading?.closest(".trading-desk")), theme: document.documentElement.dataset.theme, bodyBackground: getComputedStyle(document.body).backgroundColor, width: innerWidth, documentWidth: document.documentElement.scrollWidth, hasContent: (document.querySelector("main")?.textContent?.trim().length ?? 0) > 100, overlay: Boolean(document.querySelector('[data-nextjs-dialog], .vite-error-overlay')), overflowing: [...document.querySelectorAll("main *")].filter(e => e.getBoundingClientRect().right > innerWidth + 2 && getComputedStyle(e).position !== "fixed").slice(0, 8).map(e => ({ tag: e.tagName, class: e.className, right: e.getBoundingClientRect().right })) };
      });
      const problems = [];
      ...
      const expectedBackground = theme === "dark" ? "rgb(17, 20, 22)" : "rgb(243, 240, 233)";
      if (snapshot.bodyBackground !== expectedBackground) problems.push("shared canvas mismatch");
    ...
    // Navigation must retain the same design after client-side route changes.
  ...
  console.log(`DESIGN_SYSTEM_OK: ${results.length} page/theme/viewport combinations; shared typography, canvas, navigation and overflow checks passed.`);
