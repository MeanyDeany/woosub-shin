...
function loadStaticModule(relativeFilename) {
  ...
  const localRequire = specifier => specifier.startsWith("@/") ? loadStaticModule(`${specifier.slice(2)}.ts`) : requireDependency(specifier);
...
test("sitemap and metadata include canonical portfolio routes and no language alternates", () => {
  ...
  for (const entry of entries) { assert.equal(entry.alternates, undefined); assert.doesNotMatch(new URL(entry.url).pathname, /^\/ko(?:\/|$)/); }
...
test("all 13 retired Korean URLs retain direct temporary redirects", async () => {
  const redirects = (await nextConfig.redirects()).filter(entry => /^\/ko(?:\/|$)/.test(entry.source));
  const sources = ["/ko", "/ko/research", "/ko/papers", "/ko/projects", "/ko/projects/btc-futures-research", "/ko/projects/btc-futures-research/live-position", "/ko/projects/btc-regime-challenger", "/ko/projects/multi-asset-research-lab", "/ko/projects/multi-asset-research-lab/claims", "/ko/projects/volatility-regime-filtering", "/ko/projects/bitcoin-bubble-gsadf", "/ko/contact", "/ko/build-log"];
  assert.deepEqual(redirects.map(entry => entry.source).sort(), sources.sort());
...
test("the former program URL redirects only to canonical ASRA", async () => {
  const redirects = await nextConfig.redirects();
  assert.equal(redirects.length, 14);
  assert.equal(new Set(redirects.map(entry => entry.source)).size, redirects.length);
  assert.deepEqual(redirects.filter(entry => !/^\/ko(?:\/|$)/.test(entry.source)), [{ source: "/astra", destination: "/asra", permanent: false }]);
  for (const redirect of redirects) { assert.ok(siteRoutes.includes(redirect.destination)); assert.ok(!redirects.some(entry => entry.source === redirect.destination)); }
});
test("normal navigation exposes personal Trading and no retired public experience", () => {
  ...
  assert.deepEqual(primaryNavigation.map(item => item.label), ["Trading", "Research", "Papers", "Systems", "About"]);
