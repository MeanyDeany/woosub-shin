...
test("CSV backfill has the frozen complete UTC export window and exact aggregate", () => {
  assert.equal(CSV_REALIZED_BACKFILL_START, "2026-08-01");
  assert.equal(CSV_REALIZED_BACKFILL_END, "2026-09-22");
  assert.equal(CSV_REALIZED_BACKFILL.length, 53);
  assert.equal(
...
test("backfill replaces only MISSING rows inside the frozen CSV window", () => {
  const closed = {
    date_utc: "2026-08-20",
    status: "CLOSED",
  ...
  assert.equal(result[0].return_pct, null);
  assert.equal(result[1].source, "LEDGER");
  assert.equal(result[1].net_pnl, 999);
  assert.equal(result[2].source, "CSV_REALIZED");
  assert.equal(result[2].net_pnl, -9.3542512);
  assert.equal(result[3].source, "LEDGER");
