...
export default function BtcFuturesResearchPage() {
  return (
    <PageShell>
      <section className="mx-auto max-w-6xl px-5 py-16 sm:py-20 lg:px-8">
        ...
        <div className="mt-9 grid gap-px overflow-hidden rounded-lg border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
          {heroBoundaries.map((item) => (
            <p
              key={item}
              className="bg-neutral-950 px-4 py-4 text-sm leading-6 text-neutral-300"
            >
              {item}
            </p>
          ))}
        ...
        <p className="mt-5 max-w-4xl text-sm leading-6 text-neutral-500">
          Model performance is descriptive research evidence. It is never presented
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <Card>
            <p className="text-lg leading-8 text-neutral-300">
              The system is designed to reduce emotional and discretionary trading
              decisions by preserving factual research history. Forecast generation,
              realized outcomes, and operational checks remain distinct so each piece
              of evidence can be reproduced and audited without rewriting prior
              outputs.
            </p>
            <p className="mt-5 leading-7 text-neutral-400">
              Its role ends at evidence generation. Strategy interpretation and any
              form of execution sit outside the system boundary.
            </p>
          </Card>
          <Card>
      ...
      <PageSection eyebrow="Forecast layer" title="Shadow Models">
        ...
        <div className="mt-6 rounded-lg border border-emerald-300/30 bg-emerald-300/[0.05] px-5 py-5">
          <p className="max-w-5xl text-sm leading-7 text-neutral-200">
            These models produce raw one-hour variance forecasts only. Their outputs
            are not used as market regimes, direction forecasts, entries, vetoes,
            position sizing, leverage decisions, or trading permissions.
          </p>
        </div>
      </PageSection>

      <PageSection eyebrow="Auditability" title="Immutable Event Architecture">
        <div className="rounded-lg border border-white/10 bg-neutral-900 p-5 sm:p-6">
          <div
            className="grid gap-3 md:grid-cols-[1fr_auto_1fr_auto_1fr] md:items-center"
            aria-label="Fit ledger to state ledger to forward outcome ledger"
          >
            {[
              "Fit Ledger",
              "State Ledger",
              ...
              <div key={ledger} className="contents">
                {index > 0 ? (
                  <span
                    aria-hidden="true"
                    className="hidden text-center text-neutral-600 md:block"
                  >
                    →
                  </span>
                ) : null}
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {ledgerProperties.map((property) => (
            <div
              key={property}
              className="rounded-lg border border-white/10 px-4 py-3 text-sm leading-6 text-neutral-300"
            >
              {property}
            </div>
          ))}
        ...
        <p className="mt-6 max-w-4xl leading-7 text-neutral-400">
          The ledger chain makes provenance, concurrency decisions, and failure
          states inspectable. Historical evidence stays immutable even as research
      ...
      <PageSection eyebrow="Research operations" title="Operational Scheduler">
        ...
        <p className="mt-6 rounded-lg border border-white/10 bg-neutral-900 px-5 py-4 text-sm leading-6 text-neutral-300">
          Operational health is research infrastructure evidence, not a trading
          signal.
        </p>
      </PageSection>
      ...
      <PageSection eyebrow="2026-07-13 UTC" title="Static Operational Snapshot">
        ...
        <p className="mt-6 max-w-4xl text-sm leading-6 text-neutral-500">
          Static portfolio snapshot. This is not a live server monitor, model
          ranking, strategy approval, or trading-readiness indicator.
        </p>
      </PageSection>
      ...
      <PageSection eyebrow="Engineering scope" title="What This Project Demonstrates">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {demonstratedCapabilities.map((capability) => (
            <div
              key={capability}
              className="rounded-lg border border-white/10 bg-white/[0.04] px-4 py-4 text-sm leading-6 text-neutral-300"
            >
              {capability}
            </div>
          ))}
        ...
        <Card className="mt-6">
          ...
          <p className="mt-4 text-lg leading-8 text-neutral-200">
            Python · pandas · NumPy · arch · SQLite · CSV event ledgers · Linux ·
            cron · flock · GitHub · AWS Lightsail
          </p>
        </Card>
      </PageSection>
